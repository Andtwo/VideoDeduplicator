...
- 影视名称：必填，显示在输出画面左上角
- 输出画幅支持多选原画幅、4:3、16:9、9:16；每个任务等概率抽取一种，使用缩放加黑边保持画面完整
- 贴纸布局支持多选四角、顶部连续横条、左右连续竖条；启用贴图步骤时，每个任务从所选布局的非空组合中等概率抽取一种（三项全选共七种组合），抽取结果写入任务快照
- 保存当前版本会更新所选版本的配置与说明，保留版本号；已提交任务的快照不变
- 支持多选批量删除策略版本，当前启用版本不可删除
...
PV 按页面实例和日期去重，UV 按浏览器本地随机标识去重；不保存 IP、设备指纹。
在线人数按最近 90 秒可见页面心跳计算，不代表登录用户数。任务指标按提交日期归属。
下载率统计成功任务中至少请求过一次下载的占比，文件请求不等于下载完成或保存相册。
现存任务在启动时补录；已被清理的历史任务无法恢复，上线前的访问和下载不补算。
统计采用尽力记录：数据库忙或故障时不阻止视频提交、处理和下载；心跳与下载计数可能少记。现存任务状态每 10 分钟及启动时补录，超过任务保留期的故障数据可能无法补齐。报表不可用时返回 503。
P95 使用最近秩定义，报表在数据库中聚合，最近任务只读取 100 条，策略排行最多 100 项。
仅新任务记录精确处理耗时；失败类型为异常类名。清除浏览器存储会改变匿名访客标识。
